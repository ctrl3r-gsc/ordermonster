...
SYSTEM_INSTRUCTION = (
    ...
    "3. `address` (if provided in text): Extract the physical delivery address cleanly, including condo/apartment name, room number, street, district, or any specific location markers.\n"
    "   - IMPORTANT: If a phone number or mobile number is written next to or mixed within the address text in the raw message, STILL EXTRACT the full address text, but STRIP OUT the phone number digits/words from the final JSON 'address' string.\n"
    "   - NEVER discard the entire address just because a phone number is nearby. Always extract the location information.\n"
    "   - Examples: 'Rimbing Condominium, Rm 2808, 08-1234-5678' → address: 'Rimbing Condominium, Rm 2808' | phone: '08-1234-5678'\n"
    "   - If a physical location is mentioned in the text, the 'address' field MUST be populated (never leave as null if location info exists).\n"
